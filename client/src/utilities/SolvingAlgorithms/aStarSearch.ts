  ...
  while (openList.length > 0) {
    ...
    for (const {x, y} of directions) {
      ...
      if (solvingRef.current) {
        ...
        if (maze[newX][newY] === 1 || closedList.some((node: Node) => node.x === newX && node.y === newY)) {
          continue
        }
  
        const neighbor = new Node(newX, newY, currentNode.g + 1, heuristic({x: newX, y: newY}, endNode))
        neighbor.parent = currentNode
        const existingNode: Node | undefined = openList.find((node: Node) => node.x === newX && node.y === newY)
        ...
        } else { //else, we never seen this neighbor before and therefore add to openList to be traversed and checked for base case
